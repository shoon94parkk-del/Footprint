...
const steps=['발명자·출원인 기준으로 특허를 직접 찾는 중…','공개 웹·블로그·SNS 흔적을 확인하는 중…','회사·학교·지역·닉네임으로 동명이인을 분리하는 중…','찾은 정보를 직업·관심사·활동 스타일로 재구성하는 중…'];
...
function render(data){
  ...
  $('#groups').innerHTML=order.filter(k=>r.grouped[k]?.length).map(k=>groupHtml(k,r.grouped[k])).join('')||'<div class="method-note"><b>관련성이 높은 흔적을 충분히 찾지 못했습니다.</b><p>회사·학교·지역·닉네임 중 가능한 정보를 넣고 다시 검색해보세요. 특허는 발명자와 회사가 함께 입력되면 별도로 직접 조회합니다.</p></div>';
...
form.onsubmit=async e=>{
  ...
  const timer=setInterval(()=>{idx=Math.min(idx+1,steps.length-1);pct=Math.min(pct+22,88);$('#loadingStep').textContent=steps[idx];$('#progressBar').style.width=pct+'%';},1200);
  try{
